...
func (py PythonOperator) beforeRun(form *dto.OperationForm) {
...
func (py PythonOperator) run(form *dto.OperationForm) {
...
func (py PythonOperator) judge(form *dto.OperationForm) {
	//fmt.Println("form flag:",form.Flag)
	//fmt.Println("form len:",len(form.Flag))
	if form.Flag != "" {
		form.Score = 0
		return
	}
	...
	//fmt.Println("errouput:",form.ErrorOutput)
	if esr[1:2] == "s" {
	...
	} else if form.ExpectOutput == form.RealOutput {
		form.Flag = "AC"
	} else {
		form.Flag = "WA"
		form.Score = 0
