        ...
        <div class="left-box">
          <el-row class="problem-box">
            ...
            <el-row>
              ...
              <el-row
                class="sample"
                v-bind:key="i"
                v-for="(sample, i) in problemDetail.samples"
              >
                <el-col :span="12">
          ...
          <el-row style="text-align:left;" class="answer-box">
            <el-row style=";margin: 15px auto 15px;width:95%">
              ...
              >
                <el-option
                  v-for="item in problemDetail.languages"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                >
...
export default {
  ...
  async beforeCreate() {
    ...
    try {
      const { data: res } = await this.$http.post('/contest/getProblemDetail', {
  ...
  methods: {
    ...
    goStatusDetail(){
      this.$router.push({path:'/contest/result',query:{id:this.csmid}})
    },
    gostatus() {
    ...
    async getStatus(){
       if (this.status==="Judging" && this.waitTimes<50){
                  this.isJuding = true;
                  const { data: res } = await this.$http.post('/contest/getStatus', {
                  ...
                  this.waitTimes+=1
                  if (this.status==="Judging" && this.waitTimes<50 &&this.$route.path==="/contest/answer"){
    ...
    async submit() {
      ...
      try {
        ...
        this.waitTimes=0
        this.status = 'Sending';
        const { data: res } = await this.$http.post('/contest/submit', {
          code: this.code,
