...
export default {
  ...
  async mounted() {
    this.show = true;
    try {
      const { data: res0 } = await this.$http.post('/contest/getOITop10', {
      ...
      const { data: res3 } = await this.$http.post('/contest/getOIRankCount', {
        id: Number(this.$route.query.id)
      });
      if (res3.error) {
  ...
  methods: {
    ...
    countDuration(val) {
      var s = new Date(this.startTime.replace(/-/g, '/'));
