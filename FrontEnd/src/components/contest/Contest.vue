    ...
    <transition name="slide-fade">
      <div class="center-box" v-if="show">
        <el-row style="height:62px">
          <span
            style="float:left;font-size:23px;margin-left:30px;margin-top:15px"
            >Contests</span
          >
          <el-button
        ...
        <div v-loading="contestLoading" style="min-height:80px">
          <div v-if="contest.length===0" style="float:left;font-size:20px;width:100%;text-align:center;margin-top:25px">No Data</div>
          <el-row
            v-bind:key="index"
          ...
          >
            <img
              style="width: 45px; height: 45px;float:left;margin-left:30px;margin-top:10px"
            ...
            <el-row v-if="index!==0" style="height:1px;float:top;margin-top:-5px;border-top:1px solid rgb(233, 233, 235);">
            </el-row>
          </el-row>
        </div>
        <el-pagination
  ...
  methods: {
    ...
    async queryList() {
      try {
        ...
        const { data: res } = await this.$http.post('/contest/getAll',{
          page: this.page,
          rule: this.rule,
          status: this.status,
          keywords: this.keywords
        });
        const { data: res1 } = await this.$http.post('/contest/getCount',{
          rule: this.rule,
          status: this.status,
          keywords: this.keywords
        });
