  ...
  <div>
    <transition name="slide-fade">
      <div class="center-box" v-if="show">
            ...
            <div style="height:100%;width:80%;text-align:left">
              ...
              <p style="font-size:14px">
                ...
                <span style="margin-right:20px"
                  >Problem:&nbsp;{{ result.problemName }}</span
                >
                <span style="margin-right:20px"
                  >Author:&nbsp;{{ result.username }}</span
                >
                <span style="margin-right:20px"
                  >Language:&nbsp;{{ result.language }}</span
                >
                <span style="margin-right:20px"
                  >Score:&nbsp;{{ result.totalScore }}</span
                >
              </p>
            </div>
          </el-col>
        ...
        <el-row
          style="background-color:#ffffff;margin-top:20px;border-radius: 4px"
        >
          <el-row>
            <span
              style="font-size:20px;margin-top:15px;margin-left:20px;float:left"
              v-if="!option"
              >Status Detail</span
            >
            <span
            ...
            >
            <el-switch
              style="display: block;float:right;margin-top:15px;margin-right:2%;"
              v-model="option"
              active-color="#409eff"
          ...
          <div class="resultCode">
            <codemirror
              ref="myCm"
              style="margin-top:15px;margin-left:2%;width:98%;min-height:0px!important"
              v-if="option"
              :value="result.code"
              :options="cmOptions"
            >
...
export default {
  ...
  async beforeCreate() {
    try {
      ...
      let { data: res0 } = await this.$http.post('/practice/getStatus', {
        id: Number(psmid)
      });
      if (res0.error) {
  ...
  methods: {
    ...
    async getStatusDetail() {
      const { data: res } = await this.$http.post('/practice/getStatusDetail', {
        id: Number(this.$route.query.id)
      });
      if (res.error) {
        this.$message.error(res.error);
