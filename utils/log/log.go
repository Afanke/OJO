...
func (f *FileLogger) LogBG() {
	...
	for {
		...
		case log := <-f.logChan:
			...
			case DEBUG:
				if f.debug == nil {
					Print(ERROR, "can't open file:%s/debug.log", cfg.FilePath)
					break
				}
				...
				if err != nil {
					Print(ERROR, "check file stat error:%v", err)
					break
				}
