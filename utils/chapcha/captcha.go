...
func (captcha *Captcha) setFont(gc *draw2dimg.GraphicContext) {
	...
	fontBytes, err := ioutil.ReadFile(captcha.FontPath)
	if err != nil {
		log.Println(err)
		return
	}
	...
	gc.FontCache.Store(draw2d.FontData{Name: captcha.FontPath, Family: 0, Style: draw2d.FontStyleNormal}, font)
	gc.SetFontData(draw2d.FontData{Name: captcha.FontPath, Style: draw2d.FontStyleNormal})
